...
5. A published image is not deployment authority. Deploy only through a separate immutable
   `server-state-*` release; operator-local Harness acceptance tests that release on stage before
   separately owner-authorized production promotion.
