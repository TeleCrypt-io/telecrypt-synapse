...
readonly CANONICAL_REPOSITORY='TeleCrypt-io/telecrypt-synapse'
readonly CANONICAL_URL='https://github.com/TeleCrypt-io/telecrypt-synapse.git'
...
[[ $# -ge 1 ]] || die 'operation or refspec is required'
operation="$1"
case "$operation" in
  check|fetch|ls-remote|local-read|local-ancestor)
    shift
    ;;
  refs/*|+refs/*)
    operation=fetch
